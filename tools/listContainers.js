...
export async function execute({
  ...
  try {
    ...
    if (format === "json" && !quiet) {
      try {
        ...
        containers = stdout.trim().split('\n')
         ...
         chatService.errorLine((`[listContainers] Error parsing JSON output: ${e.message}`));
    ...
    return {
      ...
      count: Array.isArray(containers) ? containers.length : (stdout.trim().split('\n').filter(line => line.trim()).length),
      stdout: stdout?.trim() || "",
      stderr: stderr?.trim() || ""
    };
  } catch (err) {
...
export const parameters = z.object({
  ...
  filter: z.string().describe("Filter output based on conditions provided").optional(),
