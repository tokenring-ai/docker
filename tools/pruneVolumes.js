...
export async function execute({
  ...
  try {
    ...
    if (deletedMatch) {
     ...
     chatService.systemLine((`[pruneVolumes] Successfully pruned unused Docker volumes. Space reclaimed: ${spaceReclaimed}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
...
export const parameters = z.object({
  filter: z.string().describe("Filter volumes based on conditions provided").optional(),
