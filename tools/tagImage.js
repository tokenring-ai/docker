...
export async function execute({
  ...
  if (!sourceImage || !targetImage) {
     chatService.errorLine(("[tagImage] sourceImage and targetImage are required"));
  ...
  const cmd = `timeout ${timeout}s ${dockerCmd} tag ${shellEscape(sourceImage)} ${shellEscape(targetImage)}`;
   ...
   chatService.infoLine((`[tagImage] Tagging image ${sourceImage} as ${targetImage}...`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 1024 * 1024 });
     chatService.systemLine((`[tagImage] Successfully tagged image ${sourceImage} as ${targetImage}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
