 ...
 * @param {object} args
 * @param {string|string[]} args.containers - Container name(s) or ID(s)
 * @param {boolean} [args.all=false] - Whether to show all containers (default shows just running)
 * @param {boolean} [args.noStream=true] - Whether to disable streaming stats and only pull one stat
 * @param {string} [args.format="json"] - Format the output (json or table)
 * @param {number} [args.timeoutSeconds=10] - Timeout in seconds
...
export async function execute({
  ...
  if (containerList.length === 0) {
     chatService.errorLine(("[getContainerStats] at least one container must be specified"));
  ...
  cmd += ` ${containerList.map(container => shellEscape(container)).join(' ')}`;
   ...
   chatService.infoLine((`[getContainerStats] Getting stats for container(s): ${containerList.join(', ')}...`));
    ...
    if (format === "json") {
      try {
        ...
        stats = stdout.trim().split('\n')
         ...
         chatService.errorLine((`[getContainerStats] Error parsing JSON output: ${e.message}`));
        stats = stdout.trim();
      }
    } else {
      stats = stdout.trim();
     ...
     chatService.systemLine((`[getContainerStats] Successfully retrieved stats for container(s): ${containerList.join(', ')}`));
    return {
      ok: true,
      exitCode: exitCode,
      stats: stats,
