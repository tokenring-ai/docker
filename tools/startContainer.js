...
export async function execute({
  ...
  if (containerList.length === 0) {
     chatService.errorLine(("[startContainer] at least one container must be specified"));
  ...
  cmd += ` ${containerList.map(container => shellEscape(container)).join(' ')}`;
   ...
   chatService.infoLine((`[startContainer] Starting container(s): ${containerList.join(', ')}...`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 1024 * 1024 });
     ...
     chatService.systemLine((`[startContainer] Successfully started container(s): ${containerList.join(', ')}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
...
export const parameters = z.object({
  ...
  attach: z.boolean().optional().default(false).describe("Whether to attach STDOUT/STDERR and forward signals"),
  interactive: z.boolean().optional().default(false).describe("Whether to attach container's STDIN"),
