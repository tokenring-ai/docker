...
export async function execute({
  ...
  if (!container || !command) {
     chatService.errorLine(("[execInContainer] container and command are required"));
  ...
  cmd += ` ${commandList.map(arg => shellEscape(arg)).join(' ')}`;
   ...
   chatService.infoLine((`[execInContainer] Executing command in container ${container}...`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 5 * 1024 * 1024 });
     ...
     chatService.systemLine((`[execInContainer] Command executed successfully in container ${container}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
...
export const parameters = z.object({
  ...
  interactive: z.boolean().optional().default(false).describe("Whether to keep STDIN open even if not attached"),
  tty: z.boolean().optional().default(false).describe("Whether to allocate a pseudo-TTY"),
  workdir: z.string().optional().describe("Working directory inside the container"),
  env: z.record(z.string()).optional().default({}).describe("Environment variables to set"),
  privileged: z.boolean().optional().default(false).describe("Whether to give extended privileges to the command"),
  user: z.string().optional().describe("Username or UID to execute the command as"),
