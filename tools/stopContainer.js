...
export async function execute({
  ...
  if (containerList.length === 0) {
     chatService.errorLine(("[stopContainer] at least one container must be specified"));
  ...
  if (time !== 10) { // Only add if different from default
  ...
  cmd += ` ${containerList.map(container => shellEscape(container)).join(' ')}`;
   ...
   chatService.infoLine((`[stopContainer] Stopping container(s): ${containerList.join(', ')}...`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 1024 * 1024 });
     ...
     chatService.systemLine((`[stopContainer] Successfully stopped container(s): ${containerList.join(', ')}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
