 ...
 * @param {object} args
 * @param {string} args.server - The registry server URL (e.g., "https://index.docker.io/v1/")
 * @param {string} args.username - Username for the registry
 * @param {string} args.password - Password for the registry
 * @param {string} [args.email] - Email for the registry account
 * @param {boolean} [args.passwordStdin=false] - Take the password from stdin
...
export async function execute({
  ...
  if (!server || !username || (!password && !passwordStdin)) {
     chatService.errorLine(("[authenticateRegistry] server, username, and password are required"));
  ...
  if (email) {
   ...
   chatService.infoLine((`[authenticateRegistry] Authenticating to registry ${server}...`));
  // Don't log the full command as it may contain sensitive information
   chatService.infoLine((`[authenticateRegistry] Executing: ${dockerCmd} login ${server} -u ${username} [password hidden]`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, ...execOptions, timeout: timeout * 1000 });
     chatService.systemLine((`[authenticateRegistry] Successfully authenticated to registry ${server}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
...
export const parameters = z.object({
  ...
  passwordStdin: z.boolean().describe("Take the password from stdin").default(false).optional(),
  timeoutSeconds: z.number().int().describe("Timeout in seconds").default(30).optional(),
});
