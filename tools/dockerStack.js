...
export async function execute({ action, stackName, composeFile, timeoutSeconds = 60 }, registry) {
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 1024 * 1024 });
     chatService.systemLine((`[dockerStack] Successfully executed ${action} on stack ${stackName}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
...
export const description = "Launch, update, or remove a Docker stack from the local Docker Swarm. Actions: deploy (requires composeFile), remove, ps.";
...
export const parameters = z.object({
  ...
  composeFile: z.string().describe("Path to docker-compose.yml file (required for deploy)").optional(),
  timeoutSeconds: z.number().int().describe("Timeout for the stack operation in seconds (default: 60).").optional()
});
