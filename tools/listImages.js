...
export async function execute({
  ...
  try {
    ...
    if (format === "json" && !quiet) {
      try {
        ...
        images = stdout.trim().split('\n')
         ...
         chatService.errorLine((`[listImages] Error parsing JSON output: ${e.message}`));
        images = stdout.trim();
      }
    } else {
      images = stdout.trim();
    ...
    return {
      ...
      exitCode: exitCode,
      images: images,
      count: Array.isArray(images) ? images.length : (stdout.trim().split('\n').filter(line => line.trim()).length),
      stdout: stdout?.trim() || "",
      stderr: stderr?.trim() || ""
    };
  } catch (err) {
...
export const parameters = z.object({
  ...
  filter: z.string().optional().describe("Filter output based on conditions provided"),
