...
export async function execute({
  ...
  if (containerList.length === 0) {
     chatService.errorLine(("[removeContainer] at least one container must be specified"));
  ...
  cmd += ` ${containerList.map(container => shellEscape(container)).join(' ')}`;
   ...
   chatService.infoLine((`[removeContainer] Removing container(s): ${containerList.join(', ')}...`));
  ...
  try {
    const { stdout, stderr, exitCode } = await execa(cmd, { shell: true, timeout: timeout * 1000, maxBuffer: 1024 * 1024 });
     ...
     chatService.systemLine((`[removeContainer] Successfully removed container(s): ${containerList.join(', ')}`));
    return {
      ok: true,
      exitCode: exitCode,
      stdout: stdout?.trim() || "",
